...
class RetrievalPipeline:
    ...
    async def run(
        ...
        with self.tracing_manager.span(
            ...
            retrieved = await self.retriever.retrieve(query, requested_top_k, kb_id=kb_id, tenant_id=tenant_id)
            ...
            trace.record("embedding_model_alias", getattr(self.retriever.embedding_client, "alias", None))
            if self.config.rerank_enabled:
                trace.record("rerank_model_alias", getattr(self.reranker.client, "alias", None))
    ...
    async def debug(
        ...
        contexts, trace = await self.run(query, top_k, kb_id=kb_id, tenant_id=tenant_id, session_id=session_id)
