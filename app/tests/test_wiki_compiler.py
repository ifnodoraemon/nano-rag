...
def test_wiki_compiler_writes_source_page_index_and_log(tmp_path) -> None:
    ...
    topic_page = (tmp_path / "wiki" / "topics" / "default__tenant-a--leave-policy.md").read_text(
        encoding="utf-8"
    )
    scoped_index_page = (tmp_path / "wiki" / "indexes" / "default__tenant-a.md").read_text(
        encoding="utf-8"
    )
    ...
    assert "[Leave Policy](topics/default__tenant-a--leave-policy.md)" in index_page
