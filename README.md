...
- Docker 默认启动 Bifrost，模型 alias 通过网关路由；rerank 默认启用，可用 `DISABLE_RERANK=1` 显式关闭
...
真实模型调用需要在 `docker/bifrost/.env` 中配置 provider key，例如 `GOOGLE_API_KEY`、`COHERE_API_KEY` 或 `OPENAI_API_KEY`。
...
export GENERATION_MODEL_ALIAS=gemini-2.5-flash
...
GitHub Actions 会执行后端测试、前端 lint/build、Compose 配置校验，以及 app/frontend Docker image build。
