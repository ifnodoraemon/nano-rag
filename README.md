...
`tenant_id=null`、空字符串和 `__shared__` 都表示共享工作区；共享工作区只检索共享文档，不会读取具体 tenant 的文档。指定 `tenant_id` 时只检索该 tenant 的文档。

上传接口使用稳定的逻辑来源路径，例如 `uploads/default/__shared__/policy.pdf`。Docker 会把原始上传文件持久化到 `app-upload-data` 卷里的 `/workspace/data/uploads`，用于多模态证据读取、后续重嵌入和可追溯 source path；临时批处理目录会在注入请求结束后清理。
